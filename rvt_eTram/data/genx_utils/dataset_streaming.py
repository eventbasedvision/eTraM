...
def build_streaming_dataset(dataset_mode: DatasetMode, dataset_config: DictConfig, batch_size: int, num_workers: int) \
    ...
    mode2str = {DatasetMode.TRAIN: 'train', # 'train',
    ...
    for entry in tqdm([x for x in split_path.iterdir() if x.is_dir()], desc=f'creating streaming {mode2str[dataset_mode]} datasets'):
