...
def _coco_eval(gts, detections, height, width, labelmap=("car", "pedestrian"), return_aps: bool = True):
    ...
    out_keys = ('AP', 'AP_50', 'AP_75', 'AP_S', 'AP_M', 'AP_L', 'AR_max_det=1', 'AR_max_det=10', 'AR_max_det=100')
    ...
    for i in range(1, num_classes + 1):
        #TODO: add etrap to set of dataset and print class name
        print(f"Class id - {coco_gt.getCatIds(catNms=categories[i - 1]['name'])} No. of instances - {len(coco_gt.getAnnIds(catIds=i))}")
        coco_eval.params.catIds = [i] #, 4, 5, 6, 7, 8]
        coco_eval.evaluate()
